...
def main():
  ...
  try:
    ...
    for email_id in f:
        deleted_message_count = deleted_message_count + list_and_delete_messages(service, email_id.strip('\n'))
        
    print("Total deleted messages: " + str(deleted_message_count))
    
    #list_and_delete_messages(service, "noreply@notifications.freelancer.com")
...
def list_and_delete_messages(service, emailId):
    ...
    user = "me"
    messages = service.users().messages().list(userId=user, q="from:"+emailId).execute()
    if messages['resultSizeEstimate'] == 0 :
        ...
        print(emailId + ": Deleted " + str(message_count) + (" messages" if message_count > 1 else " message" ))
